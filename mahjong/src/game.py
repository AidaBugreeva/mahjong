...
class mahjong_text(pygame.font.Font):
    ...
    def get_rendering(self):
        return self.render(self.text, True, black)
...
class mahjong_board(object):
    ...
    def generate_mur(self):
        self.mur = [mahjong_tile(a + str(b)) for a in 'stw' for b in range(1, 10)] + [mahjong_tile('d' + str(a)) for a
                                                                                      in range(1, 4)] + [
        ...
        self.mur += [mahjong_tile(a + str(b)) for a in 'stw' for b in range(1, 10)] + [mahjong_tile('d' + str(a)) for a
                                                                                       in range(1, 4)] + [
                        mahjong_tile('f' + str(a)) for a in range(1, 5)]
        [a.set_visibility(False) for a in self.mur]
    ...
    def get_next_mur_gfx_position(self):
        ...
        while True:
            ...
            for y in range(int(DISPLAY_HEIGHT / 2 + (TILE_WIDTH * wdt_bnd_rto * 16) / 2),
                           ...
                           int(-TILE_WIDTH * wdt_bnd_rto)):
                for x in range(int(DISPLAY_WIDTH / 2 - (TILE_WIDTH * wdt_bnd_rto * 17) / 2) - (TILE_HEIGHT / 2),
                               int(DISPLAY_WIDTH / 2 - (TILE_WIDTH * wdt_bnd_rto * 17) / 2) - (TILE_HEIGHT / 2) - 8,
                               -6):
                    yield x, y, 270
            for x in range(int(DISPLAY_WIDTH / 2 - (TILE_WIDTH * wdt_bnd_rto * 16) / 2),
                           int(DISPLAY_WIDTH / 2 + (TILE_WIDTH * wdt_bnd_rto * 16) / 2), int(TILE_WIDTH * wdt_bnd_rto)):
                for y in range(int(DISPLAY_HEIGHT / 2 - (TILE_WIDTH * wdt_bnd_rto * 16) / 2),
            ...
            for y in range(int(DISPLAY_HEIGHT / 2 - (TILE_WIDTH * wdt_bnd_rto * 16) / 2),
                           ...
                           int(TILE_WIDTH * wdt_bnd_rto)):
                for x in range(int(DISPLAY_WIDTH / 2 + (TILE_WIDTH * wdt_bnd_rto * 17) / 2) + (TILE_HEIGHT / 2),
                               int(DISPLAY_WIDTH / 2 + (TILE_WIDTH * wdt_bnd_rto * 17) / 2) + (TILE_HEIGHT / 2) + 8, 6):
                    yield x, y, 270
    ...
    def refresh_player_gfx(self, player=0):
        ...
        for num, tile in enumerate(self.player[player]):
            if player == 0:
                tile.rect.x = int(DISPLAY_WIDTH / 2 - (TILE_WIDTH * wdt_bnd_rto * 16) / 2) + (
                        TILE_WIDTH * hand_bnd_rto * num)
                tile.rect.y = int(DISPLAY_HEIGHT / 2 + (TILE_WIDTH * wdt_bnd_rto * 18) / 2) + TILE_WIDTH / 2
                tile.set_angle(0)
            elif player == 1:
                tile.rect.x = int(DISPLAY_WIDTH / 2 - (TILE_WIDTH * wdt_bnd_rto * 18) / 2) - (
                        TILE_HEIGHT / 2) - TILE_HEIGHT
                tile.rect.y = int(DISPLAY_HEIGHT / 2 - (TILE_WIDTH * wdt_bnd_rto * 16) / 2) + (
                        TILE_WIDTH * hand_bnd_rto * num)
                tile.set_angle(90)
            elif player == 2:
                tile.rect.x = int(DISPLAY_WIDTH / 2 + (TILE_WIDTH * wdt_bnd_rto * 16) / 2) - (
                ...
                tile.rect.x = int(DISPLAY_WIDTH / 2 + (TILE_WIDTH * wdt_bnd_rto * 20) / 2) + (
                        TILE_HEIGHT / 2) - TILE_HEIGHT
                tile.rect.y = int(DISPLAY_HEIGHT / 2 + (TILE_WIDTH * wdt_bnd_rto * 16) / 2) - (
                        TILE_WIDTH * hand_bnd_rto * num)
                tile.set_angle(270)
